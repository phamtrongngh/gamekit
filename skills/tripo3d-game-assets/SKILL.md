...
description: "Generate, texture, edit, optimize, rig, animate, and export 3D game models with the Tripo3D OpenAPI. Use this skill whenever the user mentions Tripo, text-to-3D, image-to-3D, multiview-to-3D, game-ready 3D assets, low poly or retopology, mesh segmentation or completion, PBR texturing, rigging, animation retargeting, FBX/GLB/GLTF/USDZ/OBJ/STL export, or asks an agent to create usable 3D models for games. Prefer this over bitmap image skills when the desired final output is a 3D model file, animated rig, mesh, texture pass, or engine import asset."
---
...
- Download output URLs immediately; Tripo documents model/image result URLs as short-lived.
...
- Prompt-only concept prop: use `text_to_model`. Choose `P1-20260311` for low-poly topology and cleaner structured mesh; choose H3 (`v3.1-20260211` or `v3.0-20250812`) for better prompt fidelity and advanced controls.
- One reference image: use `image_to_model`. Use H3 for high fidelity, P1 when low-poly output is the priority.
- Four directional references or higher geometry consistency: use `multiview_to_model`. The files order is `[front, left, back, right]`; front is required and at least two images are needed. A prior `generate_multiview_image` or `edit_multiview_image` task can be passed as `original_task_id`.
- Weak concept art, missing character parts, or non-rig pose: first use `generate_image` with a template such as `asset_extraction`, `character_completion`, `t_pose`, `head_extraction`, `3d_enhance`, or `variants`, then generate the model from that result.
...
- Lower poly game mesh: use direct generation with `smart_low_poly=true` where suitable, or post-process with `highpoly_to_lowpoly` (`P-v2.0-20251225`) and an explicit `face_limit`.
...
6. Download `output.model`, `output.base_model`, `output.pbr_model`, `output.rendered_image`, `output.generated_image`, and multiview URLs immediately when present.
...
- Static prop or collectible: `P1-20260311`, `texture=true`, `pbr=true`, `face_limit` around `2000-8000`, final GLB/GLTF.
